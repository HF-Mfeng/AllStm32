...
//	MF_FFT_Get_Unkown_Frequency_THD();  //这两个选哪个，怎么优化，可调
	...
	for( i = 0 ; i < ManyTimes ; i++){
		...
		Show_THD[i] = MF_FFT_Get_Unkown_Frequency_THD()  * 100 * 1.3;//* a  + Show_THD[i-1]*( 1 - a );
...
void MF_Control_GPIO( GPIO_PinState PinState1 , GPIO_PinState PinState2 , GPIO_PinState PinState3 , GPIO_PinState PinState4){
	...
	HAL_GPIO_WritePin(GPIOD,GPIO_PIN_0,PinState1);
	HAL_GPIO_WritePin(GPIOD,GPIO_PIN_1,PinState2);
	HAL_GPIO_WritePin(GPIOD,GPIO_PIN_4,PinState3);
	HAL_GPIO_WritePin(GPIOD,GPIO_PIN_5,PinState4);
